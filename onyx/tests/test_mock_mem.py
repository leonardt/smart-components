...
def test_redundancy(base, mixins, params):
    ...
    for i in range(1 << ADDR_WIDTH):
        ...
        if i == 15: tester.circuit.RDATA.expect(0)
        if i == 16: tester.circuit.RDATA.expect(1)
        if i == 32: tester.circuit.RDATA.expect(2)
        if i == 255: tester.circuit.RDATA.expect(15)
